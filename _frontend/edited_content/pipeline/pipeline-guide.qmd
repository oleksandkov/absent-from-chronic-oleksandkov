...
```
CCHS .sav files (private data storage)
         │
...
| # | File | Pattern | Output |
|---|------|---------|--------|
| 1 | `manipulation/1-ferry.R` | Ferry | private data storage + `cchs-1-raw/` Parquet backup |
| 2 | `manipulation/2-ellis.R` | Ellis | private data storage + `cchs-2-tables/` Parquet |
...
| File | Location | Cycle |
|------|----------|-------|
| `CCHS2010_LOP.sav` | private data storage | 2010–2011 |
| `CCHS_2014_EN_PUMF.sav` | private data storage | 2013–2014 |
...
|----------|----------|-------------|
| `cchs-1.sqlite` | private data storage | Raw tables: `cchs_2010_raw`, `cchs_2014_raw` |
| `cchs-1-raw/*.parquet` | private data storage | Parquet backups of raw tables |

### Ellis outputs (analysis-ready)

| Artifact | Location | Format | Description |
|----------|----------|--------|-------------|
| `cchs_analytical.parquet` | private data storage | Parquet | Main analysis dataset (`apply_sample_exclusions = TRUE` by default; 63,843 rows, 62 cols) |
| `sample_flow.parquet` | private data storage | Parquet | Exclusion audit trail (5 rows) |
| `cchs-2.sqlite` | private data storage | SQLite | Same tables as Parquet (factors as character) |

### Ellis Lane 3 outputs (clarity layer — from prior run)

| Artifact | Location | Format | Description |
|----------|----------|--------|-------------|
| `cchs_analytical.parquet` | private data storage | Parquet | Full pooled sample (mirrors Lane 2) |
| `cchs_employed.parquet` | private data storage | Parquet | Employed-respondent split |
| `cchs_unemployed.parquet` | private data storage | Parquet | Non-employed-respondent split |
| `data_dictionary.parquet` | private data storage | Parquet | Variable-level metadata / data dictionary |
| `sample_flow.parquet` | private data storage | Parquet | Exclusion audit trail |
| `cchs-3.sqlite` | private data storage | SQLite | Same tables as above (factors as character) |
